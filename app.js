...
function renderRelated(p){
 ...
 row.innerHTML=items.map(x=>`<button class="related-card" onclick="openProduct('${x.id}')"><img loading="lazy" src="${x.image}" alt="${pName(x)}"><span class="related-name">${pName(x)}</span><span class="related-price">${fmt(x.price)}</span></button>`).join('');
}
...
function syncNative(){
 ...
 if(mb){
  ...
  if(currentView==='cart'&&count>0&&!anyOverlayOpen()){
   ...
   const weight=Object.entries(cart).reduce((s,[id,q])=>{const p=PRODUCTS.find(x=>x.id===id);return s+((p?.weight??DEFAULT_WEIGHT)*q)},0);
   mb.setParams?.({color:(getComputedStyle(document.documentElement).getPropertyValue('--green').trim())||'#355847',text_color:'#ffffff'});
   mb.setText(`${tr().checkout} · ${fmt(subtotal+kgFeeSum(weight))}`);mb.show();
  } else mb.hide();
 }
}
...
function reviewCard(r){
 ...
 return `<article class="review-card"><div class="review-top"><div class="review-avatar">${r.name.charAt(0)}</div><div><div class="review-name">${r.name}</div><div class="review-stars">${stars}</div></div></div><p class="review-text">${r.text}</p></article>`;
}
function renderReviews(){
 const data=REVIEWS[lang]||REVIEWS.ru;
 document.querySelector('#reviewsEyebrow').textContent=data.eyebrow;
 document.querySelector('#reviewsTitle').textContent=data.title;
 document.querySelector('#reviewsRow').innerHTML=data.items.map(reviewCard).join('');
}
